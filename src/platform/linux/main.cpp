...
Arguments parseArguments(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (value == "--review" && i + 1 < argc) args.review = argv[++i];
            ...
            std::cout
                << "Parallels X Linux validation shell\n"
                ...
                << "  --save-dir DIR      isolated Linux development save directory\n"
...
void remapGameplayControllerLayer(px::InputState& input, SDL_GameController* controller, bool abilityLayer) {
    ...
    for (const auto button : buttons)
        setGameplayControllerButton(input, button,
            controller && SDL_GameControllerGetButton(controller, button) != 0, abilityLayer);
}
...
int main(int argc, char** argv) {
    ...
    px::linux::PresentationRenderer renderer(
