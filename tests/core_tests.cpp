...
void finishOpeningTraining(px::RuntimeSession& runtime, px::InputState& input) {
    ...
    // Entering combat intentionally plays combat_ready for 0.30 seconds.
    // Validate that authored transition first, then test the dedicated
    // positive-time combat_retreat after the ready animation has completed.
    assert(runtime.view().playerAnimation == "combat_ready");
    idleFrames(runtime, input, 4, 0.1f);

    // Arena actors must face one another rather than the center/depth of the
    // ring. Moving away keeps Rrvvfo locked on the opponent and uses the
    // dedicated combat_retreat clip rather than reverse-playing the hub run.
...
void reachRouteChoice(px::RuntimeSession& runtime, px::InputState& input) {
    ...
    idleFrames(runtime, input, 9, 0.1f);
    assert(runtime.view().choiceVisible && runtime.view().choiceTitle == "CHOOSE A ROUTE");
    assert(runtime.view().choiceOptions == std::vector<std::string>({"MAIN ROAD • FIRE / WORK", "FOREST • BELLS / NAVIGATION", "CLIFF • JUMPS / VIEW"}));
...
int main() {
    ...
    assert(ch1.primaryMap == "training_region" && ch1.openingFlow.size() == 23);
    for (const auto* scene : {"sage_object_swap_field_trial", "river_object_swap_problem", "legacy_route_choice",
                              ...
                              "roadside_encounter", "collapsed_tournament_road_detour", "lens_roadblock_reveal", "tournament_outskirts_arrival"})
    ...
    const std::vector<std::string> exactOpeningOrder{
        ...
        "collapsed_tournament_road_detour", "reach_tournament_checkpoint", "tournament_checkpoint_dialogue", "reach_lens_roadblock",
    ...
    const auto& update3CheckpointCutscene = cutscenes.get("tournament_checkpoint_dialogue");
    assert(update3CheckpointCutscene.staging[0].position.x == 1420.0f);
    const auto& update3LensCutscene = cutscenes.get("lens_manual_reaction");
    assert(update3LensCutscene.staging[0].position.x == 1650.0f);
    const auto& update3OutskirtsCutscene = cutscenes.get("tournament_outskirts_arrival");
    assert(update3OutskirtsCutscene.staging[0].position.x == 1960.0f);
    ...
    const auto& collapseDefinition = exploration.get("collapsed_tournament_road_detour");
    assert(collapseDefinition.rule == px::ExplorationRuleKind::TerrainDetour);
    assert(collapseDefinition.jumpMarkers.size() == 3);
    assert(collapseDefinition.requiredAbilityId == "objectSwap");
    {
    ...
    // Update 2: a won optional spectator pass may remain pending without blocking Story.
    {
        px::SaveData pending;
        pending.story.chapterId = "rrvvfo_ch1";
        pending.story.sceneIndex = 15;
        pending.story.checkpointId = ch1.openingFlow[15].checkpointId;
        pending.story.flags = {"ch1_lost_competitor_helped", "ch1_roadside_encounter_resolved",
                               "ch1_spectator_pass_won", "ch1_spectator_pass_pending"};
        pending.world.mapId = ch1.primaryMap;
        pending.world.position = {900.0f, 0.0f};
        px::RuntimeSession pendingRuntime(chapters, maps, cutscenes, dialogue, exploration, training, adventures);
        pendingRuntime.loadSnapshot(px::SaveCodec::deserialize(px::SaveCodec::serialize(pending)));
        const auto roundTrip = pendingRuntime.saveSnapshot();
        assert(std::find(roundTrip.story.flags.begin(), roundTrip.story.flags.end(), "ch1_spectator_pass_pending") != roundTrip.story.flags.end());
        assert(std::find(roundTrip.story.flags.begin(), roundTrip.story.flags.end(), "ch1_spectator_pass_delivered") == roundTrip.story.flags.end());
    }

    // Update 3 stable-scene migration: old checkpoint-approach saves keep their logical scene.
    {
        px::SaveData preUpdate3;
        preUpdate3.schemaVersion = 5;
        preUpdate3.story.chapterId = "rrvvfo_ch1";
        preUpdate3.story.sceneIndex = 15; // old numeric position before the detour insertion
        preUpdate3.story.checkpointId = "rrvvfo-ch1-checkpoint-approach";
        preUpdate3.world.mapId = ch1.primaryMap;
        preUpdate3.world.position = {900.0f, 0.0f};
        px::RuntimeSession migratedUpdate3(chapters, maps, cutscenes, dialogue, exploration, training, adventures);
        migratedUpdate3.loadSnapshot(px::SaveCodec::deserialize(px::SaveCodec::serialize(preUpdate3)));
        assert(migratedUpdate3.view().sceneId == "reach_tournament_checkpoint");
        assert(migratedUpdate3.view().playerPosition.x >= 1340.0f);
    }
    ...
    assert(roadMap.bounds.maxX == 2100.0f);
    assert(hasPrimitive(roadPresentation, "collapse_boulder_0"));
    assert(hasPrimitive(roadPresentation, "checkpoint_post_north"));
    assert(hasPrimitive(roadPresentation, "update3_route_main_pad_0"));
    ...
    // after they have genuinely been completed once. First-time Story pacing is untouched.
    {
        ...
        // This test deliberately replays the opening scene. Omega saves prefer
        // stable checkpoint IDs over numeric indexes, so reset both identities.
        seenSave.story.checkpointId = "rrvvfo-ch1-swap-setup";
    ...
    // Both enter the existing shared Story combat and never need a platform-only mode.
    {
        ...
        assert(fightRuntime.view().pauseOptions ==
               std::vector<std::string>({"RESUME", "RESTART FIGHT", "ACCESSIBILITY & QOL", "RETURN TO TITLE"}));
    ...
    moveToward(runtime, input, {365.0f, 300.0f}, 80.0f);
    finishDialogue(runtime);
    moveToward(runtime, input, {270.0f, 0.0f}, 70.0f);
    tap(runtime, input, px::Action::Ability1);
    finishDialogue(runtime);
    for (const auto marker : exploration.get("selected_route_adventure").mainWorkMarkers)
        moveToward(runtime, input, marker, 38.0f);
    assert(runtime.view().sceneId == "swap_relay_trial");
    finishRelay(runtime, input, exploration);

    // Transport rescue is deliberately two-sided:
    // approach the stranded wheel -> Object Swap to it ->
    // acquire the dynamically-created return anchor -> Object Swap back.
    assert(runtime.view().sceneId == "transport_wheel_recovery");
    const auto& transportDefinition = exploration.get("transport_wheel_recovery");
    assert(transportDefinition.relayMarkers.size() == 2);

    moveToward(runtime, input, transportDefinition.relayMarkers[0], 180.0f);
    tap(runtime, input, px::Action::Ability2);
    assert(runtime.view().sceneId == "transport_wheel_recovery");

    const auto returnAnchor = std::find_if(
        runtime.view().worldMarkers.begin(),
        runtime.view().worldMarkers.end(),
        [](const px::RuntimeMarkerView& marker) {
            return marker.id == "transport_return" &&
                   marker.kind == "return-anchor";
        });
    assert(returnAnchor != runtime.view().worldMarkers.end());

    const px::Vec2 returnAnchorPosition = returnAnchor->position;
    moveToward(runtime, input, returnAnchorPosition, 180.0f);
    tap(runtime, input, px::Action::Ability2);
    ...
    assert(runtime.view().sceneId == "collapsed_tournament_road_detour");
    assert(runtime.view().objective == "FIND A WAY AROUND THE COLLAPSE");
    assert(runtime.view().objectiveDetail.find("OPTIONAL: Return the spectator pass") != std::string::npos);
    finishDialogue(runtime); // Rrvvfo reacts to the collapse before control resumes.
    ...
    const auto deliveredPassSave = runtime.saveSnapshot();
    assert(std::find(deliveredPassSave.story.flags.begin(), deliveredPassSave.story.flags.end(), "ch1_spectator_pass_delivered") != deliveredPassSave.story.flags.end());
    assert(std::find(deliveredPassSave.story.flags.begin(), deliveredPassSave.story.flags.end(), "ch1_spectator_pass_pending") == deliveredPassSave.story.flags.end());
    for (std::size_t detourJump = 0; detourJump < collapseDefinition.jumpMarkers.size(); ++detourJump) {
        moveToward(runtime, input, collapseDefinition.jumpMarkers[detourJump], 55.0f);
        tap(runtime, input, px::Action::Jump, 0.05f);
        if (detourJump == 0) {
            const auto midDetourSave = runtime.saveSnapshot();
            px::RuntimeSession midDetourReload(chapters, maps, cutscenes, dialogue, exploration, training, adventures);
            midDetourReload.loadSnapshot(px::SaveCodec::deserialize(px::SaveCodec::serialize(midDetourSave)));
            assert(midDetourReload.view().sceneId == "collapsed_tournament_road_detour");
            assert(std::find(midDetourReload.disabledBlockers().begin(), midDetourReload.disabledBlockers().end(), "detour_jump_gate_1") != midDetourReload.disabledBlockers().end());
            assert(std::any_of(midDetourReload.view().worldMarkers.begin(), midDetourReload.view().worldMarkers.end(), [](const px::RuntimeMarkerView& marker){ return marker.id == "collapse_jump_1" && marker.complete; }));
        }
    }
    moveToward(runtime, input, collapseDefinition.detourDashMarker, 70.0f);
    tap(runtime, input, px::Action::Dash, 0.05f);
    moveToward(runtime, input, collapseDefinition.shortcutPoint, 100.0f);
    tap(runtime, input, px::Action::Ability2, 0.05f);
    assert(px::distance(runtime.view().playerPosition, collapseDefinition.swapDestination) < 1.0f);
    moveToward(runtime, input, collapseDefinition.target, 60.0f);
    finishDialogue(runtime);
    assert(runtime.view().sceneId == "reach_tournament_checkpoint");
    moveToward(runtime, input, {1420.0f, 0.0f}, 35.0f);
    finishDialogue(runtime);
    moveToward(runtime, input, {1650.0f, 0.0f}, 35.0f);
    finishDialogue(runtime);
    ...
    assert(std::find(save.story.flags.begin(), save.story.flags.end(), "ch1_terrain_collapse_seen") != save.story.flags.end());
    assert(std::find(save.story.flags.begin(), save.story.flags.end(), "ch1_detour_dash_done") != save.story.flags.end());
    assert(std::find(save.story.flags.begin(), save.story.flags.end(), "ch1_detour_swap_done") != save.story.flags.end());
    ...
    for (const std::string route : {"forest", "cliff"}) {
        ...
        if (route == "forest") {
            for (const auto marker : exploration.get("selected_route_adventure").forestBellMarkers)
                moveToward(alternate, alternateInput, marker, 42.0f);
        }
        if (route == "cliff") {
        ...
        if (route == "cliff") {
            assert(alternate.view().dialogueVisible);
            assert(alternate.view().dialogueSpeaker == "RRVVFO");
            assert(alternate.view().dialogueText.find("view") != std::string::npos);
    ...
    // Update 6: completed optional stories remain visible at the outskirts, and
    // opening Pause does not falsely mark the current objective as completed.
    {
        const auto outskirtsStep = std::find_if(ch1.openingFlow.begin(), ch1.openingFlow.end(),
            [](const px::SceneStep& step){ return step.id == "reach_tournament_outskirts"; });
        assert(outskirtsStep != ch1.openingFlow.end());
        px::SaveData golden;
        golden.story.chapterId = "rrvvfo_ch1";
        golden.story.sceneIndex = static_cast<std::size_t>(std::distance(ch1.openingFlow.begin(), outskirtsStep));
        golden.story.checkpointId = outskirtsStep->checkpointId;
        golden.story.flags = {"ch1_lost_competitor_helped", "ch1_spectator_pass_won",
                              "ch1_spectator_pass_delivered", "ch1_cart_supplies_saved"};
        golden.world.mapId = ch1.primaryMap;
        golden.world.position = {1880.0f, 0.0f};
        golden.frontend.objectiveHistory = {"PASS THE TOURNAMENT CHECKPOINT"};

        px::RuntimeSession goldenRuntime(chapters, maps, cutscenes, dialogue, exploration, training, adventures);
        px::InputState goldenInput;
        goldenRuntime.loadSnapshot(px::SaveCodec::deserialize(px::SaveCodec::serialize(golden)));
        const auto competitor = std::find_if(goldenRuntime.view().ambientActors.begin(), goldenRuntime.view().ambientActors.end(),
            [](const px::RuntimeActorView& actor){ return actor.id == "lost_competitor"; });
        assert(competitor != goldenRuntime.view().ambientActors.end());
        assert(competitor->position.x > 1800.0f);
        const auto savedCart = std::find_if(goldenRuntime.view().worldMarkers.begin(), goldenRuntime.view().worldMarkers.end(),
            [](const px::RuntimeMarkerView& marker){ return marker.id == "saved_supply_cart"; });
        assert(savedCart != goldenRuntime.view().worldMarkers.end());
        assert(savedCart->position.x == 1815.0f);

        const std::string currentObjective = goldenRuntime.view().objective;
        ...
        assert(std::find(goldenRuntime.view().pauseSections.begin(), goldenRuntime.view().pauseSections.end(),
                         "NOW • " + currentObjective) != goldenRuntime.view().pauseSections.end());
        assert(std::find(goldenRuntime.view().pauseSections.begin(), goldenRuntime.view().pauseSections.end(),
